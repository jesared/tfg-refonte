...
export default async function AdminCommunautePage() {
  ...
  const openReports = reports.filter((report) => report.status === CommunityReportStatus.OPEN).length;
  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-6">
      <header className="rounded-2xl border border-border bg-card px-6 py-7 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Admin · Communauté</p>
        ...
        <table className="min-w-full divide-y divide-border text-sm">
          <thead className="bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
            <tr>
              ...
              <th scope="col" className="px-4 py-3 font-medium">Publication</th>
              <th scope="col" className="px-4 py-3 font-medium">Motif</th>
              <th scope="col" className="px-4 py-3 font-medium">Signalé par</th>
          ...
          <tbody className="divide-y divide-border">
            {reports.map((report) => (
              <tr key={report.id}>
                <td className="px-4 py-4 align-top text-muted-foreground">{formatDate(report.createdAt)}</td>
                <td className="max-w-md px-4 py-4 align-top">
                  <p className="line-clamp-1 font-medium text-foreground">
                    {report.post.title ?? "Publication sans titre"}
                  </p>
                  <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{report.post.content}</p>
                </td>
                <td className="px-4 py-4 align-top text-foreground">{reasonLabel[report.reason]}</td>
